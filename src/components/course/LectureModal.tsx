...
export const LectureModal = ({ isOpen, onClose, lesson, userId }: LectureModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background flex flex-col">
      ...
      {/* Video / Drive / PDF Player */}
      {(() => {
        const isDocument = /drive\.google\.com/.test(lesson.video_url) ||
          /\.pdf($|\?)/i.test(lesson.video_url) ||
          /archive\.org/.test(lesson.video_url) ||
          /docs\.google\.com\/document/.test(lesson.video_url);

        if (isDocument) {
          return (
            <div
              className="relative bg-black w-full flex-1 min-h-0 mahima-player select-none"
      ...
      {/* Collapsible Notes Section - Hidden for document content */}
      {!(/drive\.google\.com/.test(lesson.video_url) || /\.pdf($|\?)/i.test(lesson.video_url) || /archive\.org/.test(lesson.video_url) || /docs\.google\.com\/document/.test(lesson.video_url)) && (
